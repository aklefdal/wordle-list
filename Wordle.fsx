...
let findBestWord (words: string []) =
    words |> findWordValues |> Array.head |> fst
...
let findBestWord2 (baseWords: string []) (words: string []) =
    ...
    let av1 =
        wbv
        |> Array.averageBy (fun (_, v1, _, _) -> v1 |> float)

    let av2 =
        wbv
        |> Array.averageBy (fun (_, _, v2, _) -> v2 |> float)

    let av3 =
        wbv
        |> Array.averageBy (fun (_, _, _, v3) -> v3 |> float)
...
handleDuplicateCharsInAnswer [ { Char = 'm'
                                 Position = 0
                                 Color = Black }
                               { Char = 'e'
                                 Position = 1
...
let withValues = possibleWords |> findBestWord2 allWords
